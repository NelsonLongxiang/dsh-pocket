...
/**
 * 三态认证：'cookie'（权威通道）/ 'query'（?token= 兼容通道——凭据会留在
 * 地址栏与访问日志，调用方应在放行前 302 洗参）/ null（拒绝）。
 */
function authVia(req, token, sessionKey) {
  if (!token) return 'cookie'; // 无 token（未配置密码）→ 放行主通道
  const expected = cookieFor(token, sessionKey);
  const cookieTok = parseCookies(req.headers.cookie)[TOKEN_COOKIE];
  if (safeEqual(cookieTok ?? '', expected)) return 'cookie';
  const qTok = new URL(req.url ?? '/', 'http://x').searchParams.get('token');
  if (qTok != null && safeEqual(qTok, expected)) return 'query';
  return null;
}

/** 校验请求是否已认证；返回 true 放行。 */
function authCheck(req, token, sessionKey) {
  return authVia(req, token, sessionKey) !== null;
}
 ...
 */
export function createPocketProxy({ port = 13080, host = '0.0.0.0', upstream = DEFAULT_UPSTREAM, log = null, injectHtml = DEFAULT_INJECT, auth = null, rateLimit = null, heartbeat = {} } = {}) {
  ...
  const server = createServer((req, res) => {
    ...
    if (auth) {
      ...
      if (protectedHost && token) {
        ...
        const via = authVia(req, token, sessionKey);
        if (via === null) {
        ...
        if (via === 'query' && req.method === 'GET') {
          // ?token= 放行前先洗参：种持久 cookie 后 302 到去参同路径，
          // 凭据不再留在浏览器历史 / 中间人日志 / Referer 头里。
