...
/** 按代理端口派生 cookie 名：RFC 6265 的 cookie 域匹配不含端口，同一台机器上
 *  跑多个 pocket 实例（如 13080 主家 + 13081 测试家）时，固定名的 cookie 会互相
 *  覆盖——13081 一登录就把 13080 的会话顶掉线。端口后缀让每个实例的会话 cookie
 *  互不干扰（fork 特性，源自 provider-directory 线）。
 */
export function tokenCookieName(port) {
  return 'dsh_pocket_token_' + port;
}
...
/** 常量时字符串比较：长度不同直接 false（不泄露长度差以外信息），等长走 timingSafeEqual。 */
/** 校验请求是否已认证。返回 { ok, rawQueryToken }：
 *  - ok=true：已认证；rawQueryToken 非 null 表示走的是 URL `?token=<PIN>` 兼容通道
 *    （调用方应在 GET 放行前 302 洗参，非 GET 走 maybeSeedAuthCookie 种 cookie）；
 *  - 已有 cookie 命中 → rawQueryToken=null（不要重复种）。 */
function authCheck(req, tokens, sessionKey, cookieName) {
  ...
  const cookieTok = cookies[cookieName];
  if (cookieTok) {
  ...
  if (qTok) {
    ...
    // 不需要把 sha256 哈希也告诉他们。匹配上后由调用方 302 洗参（GET）或
    // maybeSeedAuthCookie 种 HttpOnly 哈希 cookie（issue #35）。
    for (const token of list) {
...
function maybeSeedAuthCookie(req, res, rawToken, sessionKey, cookieName) {
  if (!rawToken || !sessionKey) return;
  if (parseCookies(req.headers.cookie)[cookieName]) return; // 已有 cookie 就不重复种
  const expected = cookieFor(rawToken, sessionKey);
  if (!expected) return;
  const origWriteHead = res.writeHead.bind(res);
  res.writeHead = function (statusCode, headers) {
    const h = { ...(headers ?? {}) };
    const cookie = `${cookieName}=${expected}; HttpOnly; SameSite=Lax; Path=/; Max-Age=${COOKIE_MAX_AGE}`;
...
export function createPocketProxy({ port = 3081, host = '0.0.0.0', upstream = DEFAULT_UPSTREAM, log = null, injectHtml = DEFAULT_INJECT, auth = null, rateLimit = null, heartbeat = {}, lanAccessEnabled = () => true, launchToken = () => '', handshakeLimit } = {}) {
  ...
  // 会话 cookie 按端口隔离（fork 特性）。port=0（OS 随机分配）时从 server.address()
  // 惰性取真实端口——请求/升级到达时监听已建立，address() 必有值（对象）。
  let tokenCookieResolved = null;
  function tokenCookie() {
    if (tokenCookieResolved) return tokenCookieResolved;
    const a = server.address();
    tokenCookieResolved = tokenCookieName(a && typeof a === 'object' ? a.port : port);
    return tokenCookieResolved;
  }
...
upstream/main
              ...
              res.writeHead(302, {
                ...
                // fork 保留按端口隔离的 cookie 名（tokenCookie()），比较结果取上游的 matched
                'set-cookie': `${tokenCookie()}=${cookieFor(matched, sessionKey)}; HttpOnly; SameSite=Lax; Path=/; Max-Age=${COOKIE_MAX_AGE}`,
                'cache-control': 'no-store',
              });
              res.end();
            } else {
        ...
        const authResult = authCheck(req, acceptedTokens, sessionKey, tokenCookie());
        if (!authResult.ok) {
          if (isGuess) {
            limiter?.record(ip);
        ...
        // ?token=<原始 PIN> 直达（issue #35 + PR#8 洗参 + issue #90 计数语义）：
        // 正确的分享链接与 POST 登录成功同权——先清该 IP 的失败计数；
        // GET 放行前再 302 洗参，凭据不再留地址栏/日志/Referer。
        if (authResult.ok && authResult.rawQueryToken) limiter?.clear(ip);
        if (authResult.ok && authResult.rawQueryToken && req.method === 'GET') {
          const u = new URL(req.url ?? '/', 'http://x');
          u.searchParams.delete('token');
          res.writeHead(302, {
            location: u.pathname + u.search,
            'set-cookie': `${tokenCookie()}=${cookieFor(authResult.rawQueryToken, sessionKey)}; HttpOnly; SameSite=Lax; Path=/; Max-Age=${COOKIE_MAX_AGE}`,
            'cache-control': 'no-store',
          });
          res.end();
          return;
        }
        // 非 GET（API/子资源）走 writeHead 补丁种 cookie，避免「主页 200 但子资源 401」白屏
        if (authResult.rawQueryToken) maybeSeedAuthCookie(req, res, authResult.rawQueryToken, sessionKey, tokenCookie());
      }
    }
  ...
  server.on('upgrade', (req, socket, head) => {
    ...
    if (auth) {
      ...
      const wsAuth = authCheck(req, acceptedTokens, auth.sessionKey ?? null, tokenCookie());
      if (token && !wsAuth.ok) {
        if (wsGuess) {
          limiter?.record(wsIp);
