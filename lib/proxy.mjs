 ...
 * 跑多个 pocket 实例（如 13080 主家 + 13081 supervisor 家）时，固定名的
 * cookie 会互相覆盖——13081 一登录就把 13080 的会话顶掉线。端口后缀让
 * 每个实例的会话 cookie 互不干扰。
 */
...
function authCheck(req, token, cookieName) {
  if (!token) return true; // 无 token（未配置密码）→ 放行
  const cookieTok = parseCookies(req.headers.cookie)[cookieName];
...
/**
 * 公网隧道（trycloudflare.com Host）下禁止的配置面 RPC 方法（模型管理仅限局域网）。
 * 与宿主 PRIVILEGED_METHODS 同集：settings/credentials 全家 + llm.discoverModels。
 * 代理把 Host 洗成 loopback 后宿主栅栏拦不住隧道流量——这道闸在代理层补上。
 */
const TUNNEL_BLOCKED_METHODS = new Set([
  'settings.describe', 'settings.openDocument', 'settings.update', 'settings.replace', 'settings.mutate',
  'credentials.describe', 'credentials.set', 'credentials.unset',
  'llm.discoverModels',
]);

/** 请求是否来自公网隧道（按入站 Host 判定，改写前）。 */
function isTunnelHost(host) {
  return /trycloudflare\.com$/i.test(String(host ?? ''));
}

/**
 * 公网隧道下的敏感 RPC 拦截：/api/<method> 命中名单即 403。
 * 非 POST、非 /api、非隧道 Host 返回 null（放行）。
 */
function tunnelRpcGuard(req) {
  if (req.method !== 'POST' || !String(req.url ?? '').startsWith('/api/')) return null;
  if (!isTunnelHost(req.headers.host)) return null;
  const method = decodeURIComponent(String(req.url).slice('/api/'.length).split('?')[0] ?? '');
  if (!TUNNEL_BLOCKED_METHODS.has(method)) return null;
  return (res) => {
    res.writeHead(403, { 'content-type': 'application/json', 'cache-control': 'no-store' });
    res.end(JSON.stringify({ error: 'model management is LAN-only; blocked on the public tunnel' }));
  };
}
 ...
 * 启动 dsh-pocket 代理。
 * @param {object} opts
 * @param {number} [opts.port]      监听端口（默认 13080；dsh web 保持 3080）
 ...
 */
export function createPocketProxy({ port = 13080, host = '0.0.0.0', upstream = DEFAULT_UPSTREAM, log = null, injectHtml = DEFAULT_INJECT, auth = null } = {}) {
  // 会话 cookie 名带实际监听端口：同机多实例（13080/13081）互不覆盖（RFC 6265 域匹配不含端口）。
  // port=0（OS 随机分配）时也要拿到真实端口，故在请求时从 server.address() 惰性求值。
  let cookieName = tokenCookieName(port);
  const server = createServer((req, res) => {
    ...
    if (auth) {
      ...
      if (protectedHost && token) {
        ...
        if (!authCheck(req, token, cookieName)) {
    ...
    // 公网隧道下的配置面 RPC 拦截（模型管理仅限局域网）
    const blocked = tunnelRpcGuard(req);
    if (blocked) { blocked(res); return; }
  ...
  server.on('upgrade', (req, socket, head) => {
    ...
    if (auth) {
      ...
      if (token && !authCheck(req, token, cookieName)) {
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(port, host, () => {
      ...
      cookieName = tokenCookieName(actualPort); // port=0 随机分配后落到真实端口
      resolve({
        server,
        port: actualPort,
        close: () => new Promise((r) => {
