...
/**
 * 代理端口解析（优先级：internals.port 注入 > config.port 用户配置 > 默认推导）。
 * 默认不再是固定 13080，而是 dsh web 端口 + 10000（见 defaultPocketPort）：
 * 多实例并行（生产 3080 / 测试 3082）各自得到 13080/13082，互不抢端口。
 * @param {object} o
 * @param {number} [o.internalsPort] 宿主/测试注入端口（最高优先）
 * @param {number} [o.configPort]    用户配置端口（config.port）
 * @param {number} o.dshPort         dsh web 实际端口（推导基准）
 * @returns {number} 生效的代理端口
 */
export function resolvePocketPort({ internalsPort, configPort, dshPort } = {}) {
...
export function apply(ctx, config = {}, internals = {}) {
  ...
  const service = internals.service ?? createPocketService({
    dshPort,
    port: resolvePocketPort({ internalsPort: internals.port, configPort: config.port, dshPort }),
    home: internals.home,
    internals,
