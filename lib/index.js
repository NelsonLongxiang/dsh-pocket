...
import { createPocketService, defaultPocketPort } from './service.mjs';
...
/**
 * 代理端口解析（优先级：internals.port 注入 > config.port 用户配置 > settings.proxyPort
 * > 默认推导）。默认不再是固定端口，而是 dsh web 端口 + 10000（见 defaultPocketPort）：
 * 多实例并行（生产 3080 / 测试 3081、3082）各自得到 13080/13081/13082，互不抢端口。
 * @param {object} o
 * @param {number} [o.internalsPort] 宿主/测试注入端口（最高优先）
 * @param {number} [o.configPort]    cordis patch 配置端口（config.port）
 * @param {number} [o.settingsPort]  settings.json 的 proxyPort（issue #70）
 * @param {number} o.dshPort         dsh web 实际端口（推导基准）
 * @returns {number} 生效的代理端口
 */
export function resolvePocketPort({ internalsPort, configPort, settingsPort, dshPort } = {}) {
  if (Number.isInteger(internalsPort) && internalsPort > 0) return internalsPort;
  if (Number.isInteger(configPort) && configPort > 0) return configPort;
  if (Number.isInteger(settingsPort) && settingsPort > 0) return settingsPort;
  return defaultPocketPort(dshPort);
}
...
export function apply(ctx, config = {}, internals = {}) {
  ...
  const service = internals.service ?? createPocketService({
    dshPort,
    // 端口（issue #70 + fork 端口推导）：internals.port > cordis config.port > settings.proxyPort
    // > 推导默认（dsh web 端口 + 10000，非法 dshPort 回退 13080）。多实例（生产 3080/测试 3081+）
    // 天然错开互不抢端口；显式配置被占时 createPocketService 内部 EADDRINUSE 抛错（fail-fast，
    // 按插件启动失败处理：改 settings.json 的 proxyPort 后重启即可），不做静默端口漂移。
    port: resolvePocketPort({ internalsPort: internals.port, configPort: config.port, settingsPort: proxyPort() || null, dshPort }),
    home: internals.home,
    internals,
  ...
  const disposers = [];
  // 0.1.5 起 connection.rpc.handle 在宿主 rpc-host 内部访问 webServer 注入，
  // 消费方必须先经 ctx.inject(['connection', 'webServer']) 取得双声明作用域
  // （上游 gateway 同款形态）；旧宿主的 ctx.inject 同样可用，行为不变。
  const pocketRpcOptions = {
    service,
    desktop: isDesktop,
  ...
  const disposeRpc = typeof ctx.inject === 'function'
    ? ctx.inject(['connection', 'webServer'], rpcCtx => installPocketRpc(rpcCtx, pocketRpcOptions))
    : installPocketRpc(ctx, pocketRpcOptions);
