...
/**
 * 默认代理端口 = dsh web 实际端口 + 10000（3080→13080，3082→13082）。
 * 按实例推导而不是全局固定端口：测试实例与生产并行时各自错开，不再抢同一个端口。
 * dshPort 非法（缺省/越界，含推导结果会撞自身端口区间的 10000+）退回 13080。
 * @param {number} dshPort dsh web 监听端口
 * @returns {number} 默认代理端口
 */
export function defaultPocketPort(dshPort) {
  const n = Number(dshPort);
  return Number.isInteger(n) && n > 0 && n < 10000 ? n + 10000 : 13080;
}
...
export function createPocketService({
  ...
  let proxy = null;
  // 端口回退事实（正式规格）：{ from, to } | null。startProxy 每次启动时重置；
  // dispose 不清除——保留最近一次回退事实供诊断（半恢复误判教训，2026-09-04）。
  ...
  return {
    ...
    /** 启动局域网代理（幂等）。端口被占（EADDRINUSE，如桌面版与普通环境同时运行）时自动尝试下一个端口。
      * 回退是正式规格（+N 步长，N≤10）：发生时高声留痕（显著日志）并把回退事实记入 proxyPortFallback，
      * 经 status() 暴露——消费者必须能分辨「设置值」与「回退值」（2026-09-04 .157 端口漂移半恢复误判教训）。 */
    async startProxy() {
    ...
    async status() {
      ...
      return {
        ...
        proxyPort,
        // 回退事实：null=端口为设置值；{from,to}=13080 被占回退到 to。UI/巡检据此高声展示。
        proxyPortFallback: proxyFallback,
        lanUrl,
