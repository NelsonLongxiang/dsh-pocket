...
test('mobile-apply 已接线 startFileGuard（窄屏生效，且不再接 fileCopy）', () => {
  assert.ok(apply.includes("import { startFileGuard } from './fileGuard.ts'"), '必须 import 模块');
  assert.ok(apply.includes('startFileGuard()'), '必须调用 startFileGuard');
  assert.ok(apply.includes("'dsh-mobile-nav: file open guard + hide add-workspace (issue #17)'"), 'effect 标签应标注 issue #17');
...
test('CSS 隐藏「添加工作区」图标（窄屏，中英双语 aria-label）', () => {
  ...
  // 复制按钮的样式必须已彻底移除
  assert.ok(!css.includes('[data-mobile-nav="copy-file"]'), '不应残留 copy-file 按钮样式');
});
