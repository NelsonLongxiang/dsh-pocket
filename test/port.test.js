...
test('resolvePocketPort: 配置优先于推导（config.port 覆盖默认）', () => {
  assert.equal(resolvePocketPort({ configPort: 14000, dshPort: 3080 }), 14000);
});

test('resolvePocketPort: internals.port 注入优先于配置', () => {
  assert.equal(resolvePocketPort({ internalsPort: 15000, configPort: 14000, dshPort: 3080 }), 15000);
});
